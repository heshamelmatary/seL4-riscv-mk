...
Arch_recycleCap(bool_t is_final, cap_t cap)
{
    ...
    case cap_small_frame_cap:
    case cap_frame_cap:
        cleRISCVemory((void *)cap_get_capPtr(cap), cap_get_capSizeBits(cap));
        Arch_finaliseCap(cap, is_final);
        return resetMemMapping(cap);

    case cap_page_table_cap: {
        pte_t *pt = PTE_PTR(cap_page_table_cap_get_capPTBasePtr(cap));
        cleRISCVemory((void *)pt, cap_get_capSizeBits(cap));
        if (cap_page_table_cap_get_capPTIsMapped(cap)) {
    ...
    case cap_page_directory_cap: {
        ...
        memzero(pdPtr, (kernelBase >> RISCVSectionBits) << PDE_SIZE_BITS);
    ...
    case cap_asid_pool_cap: {
        ...
        asid_pool_t* ptr =
            ASID_POOL_PTR(cap_asid_pool_cap_get_capASIDPool(cap));
        if (RISCVKSASIDTable[base >> asidLowBits] == ptr) {
            deleteASIDPool(base, ptr);
            memzero(ptr, sizeof(pde_t*) << asidLowBits);
            RISCVKSASIDTable[base >> asidLowBits] = ptr;
        }
        return cap;
    }
    default:
...
{
    switch (t) {
    case seL4_RISCV_SmallPageObject:
        return RISCVSmallPageBits;
    case seL4_RISCV_LargePageObject:
        return RISCVLargePageBits;
    case seL4_RISCV_SectionObject:
        return RISCVSectionBits;
    case seL4_RISCV_SuperSectionObject:
...
Arch_createObject(object_t t, void *regionBase, word_t userSize)
{
    ...
    case seL4_RISCV_LargePageObject:
        memzero(regionBase, 1 << RISCVLargePageBits);
        /** AUXUPD: "(True, ptr_retyps 16
                 (Ptr (ptr_val \<acute>regionBase) :: user_data_C ptr))" */
        /** GHOSTUPD: "(True, gs_new_frames vmpage_size.RISCVLargePage
                                            (ptr_val \<acute>regionBase)
                                            (unat RISCVLargePageBits))" */
        cleanCacheRange_PoU((word_t)regionBase,
                            (word_t)regionBase + (1 << RISCVLargePageBits) - 1,
                            addrFromPPtr(regionBase));

        return cap_frame_cap_new(
                   RISCVLargePage, ASID_LOW(asidInvalid), VMReadWrite,
                   0, ASID_HIGH(asidInvalid),
                   (word_t)regionBase);

    case seL4_RISCV_SectionObject:
        memzero(regionBase, 1 << RISCVSectionBits);
        /** AUXUPD: "(True, ptr_retyps 256
                 (Ptr (ptr_val \<acute>regionBase) :: user_data_C ptr))" */
        /** GHOSTUPD: "(True, gs_new_frames vmpage_size.RISCVSection
                                            (ptr_val \<acute>regionBase)
                                            (unat RISCVSectionBits))" */
        cleanCacheRange_PoU((word_t)regionBase,
                            (word_t)regionBase + (1 << RISCVSectionBits) - 1,
                            addrFromPPtr(regionBase));

        return cap_frame_cap_new(
                   RISCVSection, ASID_LOW(asidInvalid), VMReadWrite,
                   0, ASID_HIGH(asidInvalid),
                   (word_t)regionBase);

    case seL4_RISCV_SuperSectionObject:
        memzero(regionBase, 1 << RISCVSuperSectionBits);
        /** AUXUPD: "(True, ptr_retyps 4096
                 (Ptr (ptr_val \<acute>regionBase) :: user_data_C ptr))" */
        /** GHOSTUPD: "(True, gs_new_frames vmpage_size.RISCVSuperSection
                                            (ptr_val \<acute>regionBase)
                                            (unat RISCVSuperSectionBits))" */
        cleanCacheRange_PoU((word_t)regionBase,
                            (word_t)regionBase + (1 << RISCVSuperSectionBits) - 1,
                            addrFromPPtr(regionBase));

        return cap_frame_cap_new(
                   RISCVSuperSection, ASID_LOW(asidInvalid), VMReadWrite,
                   0, ASID_HIGH(asidInvalid),
                   (word_t)regionBase);
